...
const { getPokemonWithTraits } = require('../pokemon/helper');
...
router.get('/pokemons', (req, res, next) => {
    ...
    authenticatedAccount({ sessionString })
        .then(({ account }) => {
            return AccountPokemonTable.getAccountPokemons({ accountId: account.id });
        })
        .then(({ accountPokemons }) => {
            return Promise.all(
                accountPokemons.map(accountPokemon => {
                    return getPokemonWithTraits({ pokemonId: accountPokemon.pokemonId });
                })
            );
        })
        .then(pokemons => {
